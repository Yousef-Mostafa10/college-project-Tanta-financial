...
//   }
//
//   // 🔹 جلب الforwards للمعاملة وتحديد حالة المستخدم الحالي (Logic Updated)
...
//
//         // 1. Filter interactions involving me (Sender OR Receiver)
//         final myInteractions = forwards.where((f) {
//           final senderName = f['sender']?['name'];
//           final receiverName = f['receiver']?['name'];
//           return senderName == _userName || receiverName == _userName;
//         }).toList();
//
...
//
//         // 3. Check the LATEST interaction
//         final lastInteraction = myInteractions.last;
//
//         if (lastInteraction['receiver']?['name'] == _userName) {
//           // Case A: I am the current receiver (The ball is in my court)
//           return lastInteraction['status'] ?? 'waiting';
//         } else {
//           // Case B: I am the sender (I forwarded it)
//           // I want to see My decision (Approved/Rejected) that allowed me to forward.
//           // Fallback to 'waiting' if not found (e.g. Creator case)
//           final lastReceipt = myInteractions.lastWhere(
//             (f) => f['receiver']?['name'] == _userName,
//             orElse: () => null,
//           );
//
//           return lastReceipt?['status'] ?? 'waiting';
//         }
//       } else {
//         return 'unknown';
//       }
